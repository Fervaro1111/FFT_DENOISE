...
#include "5G_INTERLEAVER_PDCCH_interfaces.h"
#include "5G_INTERLEAVER_PDCCH_functions.h"
#include "5G_INTERLEAVER_PDCCH.h"

extern Interl_t Interleave_info;

//ALOE Module Defined Parameters. Do not delete.
char mname[STR_LEN]="5G_INTERLEAVER_PDCCH";
int run_times=1;
...
int initialize() {
	...
	if (block_length > get_input_max_samples()) {
		...
		moderror("Check your 5G_INTERLEAVER_PDCCH_interfaces.h file\n");
		return -1;
	}
	/*Include the file name and line number when printing*/
...
int work(input_t *inp, output_t *out) {
	...
	int rcv_samples_bits=rcv_samples*8;
	int snd_samples=0;
	...
	if (rcv_samples_bits> K_max) {
		printf("Exceded maximum interleaver bits\n");
		return (0);
	}
	...
	if(TX_RX==0){
		...
		do_interleave(input0,output0,&Interleave_info,rcv_samples_bits);
	}
	//RX mode
	if(TX_RX==1){

		undo_interleave(input0,output0,&Interleave_info,rcv_samples_bits);
