...
#include "DEMODULATOR_interfaces.h"
#include "DEMODULATOR_functions.h"
#include "DEMODULATOR.h"

//ALOE Module Defined Parameters. Do not delete.
char mname[STR_LEN]="DEMODULATOR";
int run_times=1;
...
int initialize() {
	...
	if (block_length > get_input_max_samples()) {
		...
		moderror("Check your DEMODULATOR_interfaces.h file\n");
		return -1;
	}
	/*Include the file name and line number when printing*/
...
int work(input_t *inp, output_t *out) {
	...
	char bitsOUT_demod[2048*50*8];
	char bytesOUT_demod[2048*50*8];
	_Complex float bytesOUT[2048*8];
	float *output0FLOAT;

	input_t *input;
	output_t *output0;
	//char output0[2048*50];
	output0=out(out,0);
	input=in(inp,0);
...
/////////Process MAC input////////////////////
	...
	int *ctrl = in(inp,1);
	int new_value=*ctrl;
	//printf("Entra del bufferRX_new_value=%d \n",new_value);
	int buff_out=buffer(MAC_buffer_size,new_value,Buffer_MAC);
	printf("SURT del bufferRX_DEL_DEMODULADOR=%d \n",buff_out);
	//printf("Entra del bufferRX=%d \n",*ctrl);
	mod_order=buff_out;
	
	printf("DEMODULATOR rcv samp=%d\n",rcv_samples);
/////////Process MAC input////////////////////
...
///////////////////////////////Modulation selection//////////////////////////
	...
	//if (mod_order==1)snd_samples=soft_demod_BPSK (input, 	rcv_samples)/8,bitsOUT_demod);

	if (mod_order==1)snd_samples=demod_BPSK (bitsOUT_demod, input, 	rcv_samples)/8;

	if (mod_order==2)snd_samples=demod_4QAM (bitsOUT_demod, input, 	rcv_samples)/8;
	
	if (mod_order==4)snd_samples=hard_demod_16QAM (bitsOUT_demod, input, 	rcv_samples)/8;
...
/*	int j=0;
		for(i=0; i<snd_samples*8; i++){
			printf("%d", bitsOUT_demod[i]);
			j++;
			if(j==256){
				j=0;
				printf("\n");
...
*/
	...
	bit2Byte(bitsOUT_demod, output0,  snd_samples);
/*	printf("DEMODOUTPUTTTT\n");
