...
#include "SEPARATE_interfaces.h"
#include "SEPARATE_functions.h"
#include "SEPARATE.h"

//ALOE Module Defined Parameters. Do not delete.
char mname[STR_LEN]="SEPARATE";
int run_times=1;
...
int initialize() {
	...
	if (block_length > get_input_max_samples()) {
		...
		moderror("Check your SEPARATE_interfaces.h file\n");
		return -1;
	}
	/*Include the file name and line number when printing*/
...
int work(input_t *inp, output_t *out) {
	 /** number of samples at itf 0 buffer */
	int snd_samples=0;
	int i,k;
	float *input,*output;
	int *inputMAC;
	...
	if(get_input_samples(1)!=0){
		num_codeblocks=*inputMAC;
	}


/*	printf("SEPARATE INN  rcv_samples=%d \n",rcv_samples);
...
if (rcv_samples == 0){
	snd_samples=cb_size;
	codebloc_counter++;	
memcpy(output,separation+codebloc_counter*snd_samples,snd_samples*sizeof(float));



/*	printf("%s OUT: snd_samples=%d\n", mname, snd_samples);
	printf("SEPARATE OUTTTT \n");
...
*/
	...
	cb_size=rcv_samples/num_codeblocks;
	

	snd_samples=cb_size;
	memcpy(separation,input,rcv_samples*sizeof(float));
	memcpy(output,separation,snd_samples*sizeof(float));
	
printf("%s OUT: snd_samples=%d\n", mname, snd_samples);
	printf("%s IN: rcv_samples=%d\n", mname, rcv_samples);

/*	printf("SEPARATion",rcv_samples);
	for(i=1; i<rcv_samples+1; i++){
		printf("%f", (separation[i-1]));
		//if(i%80==0)printf("\n");
	}
	printf("\n");
*/
