...
#include "5G_RATE_MATCH_PDSCH_functions.h"
#include "5G_RATE_MATCH_PDSCH_interfaces.h"
...
/*	printf("CODEBLOCKS IN UN-RM   NofCodeBlocks=%d, CodeBlockSize=%d, CodeBlockDataSize=%d, Zc=%d, ILS=%d, graph=%d\n", 
...
//////////////DO RM //////////////7	
	...
	float out_deinterleave[OUTPUT_MAX_DATA];
	int r;
	int out_position[OUTPUT_MAX_DATA];
	float pre_output[OUTPUT_MAX_DATA];
	...
	for (r=0;r<RM_info.C;r++){
		...
		//memcpy(out_deinterleave,input+r*rcv_samples/(RM_info.C),rcv_samples*sizeof(float));
		...
		*/
		...
		memcpy(outputF+OUTPUT_MAX_DATA*r/sizeof(float),pre_output,rcv_at_RM*sizeof(float));
...
//		printf("RATE MATCH E[r]=%d\n",Er[r]);
		char *pter;
		char out_selection[OUTPUT_MAX_DATA];
		char out_interleave[OUTPUT_MAX_DATA];
		char output_casi[OUTPUT_MAX_DATA];

	for (r=0;r<RM_info.C;r++){
		
		bit_selection(Er[r], INbytes+INPUT_MAX_DATA*r, out_selection, RM_info.N, k0, rcv_samples);
...
void rate_MATCH_calc_punct(int Er[],char INbytes[],char* output,int rcv_samples,int k0,infoRM_t RM_info){
	...
	int i;
		...
		char in_punctured[2048*8*10];
		char tmp[2048*8*10];
	for (r=0;r<RM_info.C;r++){
		//Trnaspose(RM_info,INbytes+80*2048*r,&tmp);
		puncture_initial_cols(INbytes+OUTPUT_MAX_DATA*r, &in_punctured,RM_info,RM_info.N);
		bit_selection(Er[r], &in_punctured, &out_selection, RM_info.N, k0, rcv_samples);
...
//////////////DO RM //////////////7	
	...
	float out_depuncture[2048*8*10];
	int rcv_at_RM=RM_info.N;
	//printf("rcv_at_RM=%d\  CB_size=%dn",rcv_at_RM,RM_info.CB_size);
	...
	for (r=0;r<RM_info.C;r++){
		...
		*/
		//memcpy(floats_out+Er[r]*r,output_casi,Er[r]*sizeof(float));
		//de_Trnaspose(RM_info,&pre_output,&out_depuncture);
		de_puncture_initial_cols(&pre_output,&out_depuncture,RM_info,RM_info.N);
				
		memcpy(outputF+OUTPUT_MAX_DATA*r/sizeof(float),&out_depuncture,rcv_at_RM*sizeof(float));			
