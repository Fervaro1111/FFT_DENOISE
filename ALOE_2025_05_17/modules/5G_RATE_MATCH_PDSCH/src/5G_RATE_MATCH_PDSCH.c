...
char mname[STR_LEN]="5G_RATE_MATCH_PDSCH";
...
int initialize() {
	...
	if(TX_RX==RX){
		...
		printf("O    SPECIFIC PARAMETERS SETUP: \033[1;34m%s\033[0m\n", mname);
		printf("O      Nof Inputs=%d, DataTypeIN=%s, Nof Outputs=%d, DataTypeOUT=%s\n", 
				   1, "FLOATs", NOF_OUTPUT_ITF, "FLOATs");
	("O--------------------------------------------------------------------------------------------O\n");
...
int work(input_t *inp, output_t *out) {
	...
	if(TX_RX==TX){
		...
		/////////////////DO RM //////////////
/*		printf("rv=%d, TB_sizeCRC=%d, mod_qam=%d, coderate=%f, REs_DMRS=%d, Free RE=%d NumCodeblocks=%d\n" , 
			RM_info.rv, RM_info.TB_TBCRC, RM_info.mod_order, RM_info.coderate, RM_info.REs_DMRS,RM_info.free_RE, RM_info.C);
*/
...
//		printf("%s OUT: snd_samples=%d\n", mname, snd_samples);
		...
		//for(int i=0;i<RM_info.C;i++)set_output_samples(i, snd_samples);
...
*/
		...
		//printf("%s: Tslot=%d\n",mname, Ts);	
		Calc_CB(&RM_info);
		//printf("%s:Tslot=%d,  NofCodeBlocks=%d, CodeBlockSize=%d, Zc=%d, ILS=%d, graph=%d\n", 
		//			mname, Ts, RM_info.C, RM_info.CB_size, RM_info.Zc, RM_info.ILS2, RM_info.graph);
...
//		printf("UN RATE MATCH TS=%d, G=%d,k0=%d\n",Ts,G,k0);
		...
		for(int i=0;i<RM_info.C;i++){
			set_output_samples(i,snd_samples*sizeof(float));
		}
