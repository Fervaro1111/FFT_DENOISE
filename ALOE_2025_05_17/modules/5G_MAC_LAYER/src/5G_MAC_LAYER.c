...
#include "5G_MAC_LAYER_interfaces.h"
#include "5G_MAC_LAYER_functions.h"
#include "5G_MAC_LAYER.h"
...
char mname[STR_LEN]="5G_MAC_LAYER";
...
print_t T;
int textcolor=16, backcolor=155, noflines;
...
int initialize() {
	...
	sprintf(&T.text[2][0]," CODERATE =%f\n",coderate);
	sprintf(&T.text[3][0]," CODERATEMCS =%f\n",MACinfo.coderateMCS);
	sprintf(&T.text[4][0]," QAM_order=%d\n",MACinfo.QAM_order);
	sprintf(&T.text[5][0]," num_layers=%d\n",MACinfo.num_layers);
	sprintf(&T.text[6][0]," Overhead_higherlayer=%d\n",MACinfo.Overhead_higherlayer);
	sprintf(&T.text[7][0]," nPRB=%d\n",MACinfo.nPRB);
	sprintf(&T.text[8][0]," RNTI=%d\n",MACinfo.RNTI);
	sprintf(&T.text[9][0]," CELLID=%d\n",MACinfo.CELLID);
	sprintf(&T.text[10][0]," q=%d\n",MACinfo.q);
	sprintf(&T.text[11][0]," rv=%d\n",MACinfo.rv);
	sprintf(&T.text[12][0]," DMRS_type=%d\n",MACinfo.DMRS_type);
	sprintf(&T.text[13][0]," Additional_DMRS_position=%d\n",MACinfo.Additional_DMRS_position);
	sprintf(&T.text[14][0]," CORESETS_symbol_size=%d\n",MACinfo.CORESETS_symbol_size);
	sprintf(&T.text[15][0]," Agr_lvl_DCI=%d\n",MACinfo.Agr_lvl_DCI);

	noflines=16;
	// printTEXT(noflines, textcolor, backgroundcolor, border_char)
	printTEXT(noflines, textcolor, backcolor, '=');

	MACinfo.NID2=MACinfo.CELLID%3;
	MACinfo.NID1=MACinfo.CELLID/3;
...
//AGB
	...
	if(Period == MAXPERIOD){
		...
		for(i=0; i<NOF_OUTPUT_ITF; i++)set_output_samples(i,0);
		return(0);
	}
	...
	if(TX_RX==0){
		...
		if (Ts==0 || Ts==1 ){
			...
			TBS_short=Calc_TBS(Symb_X_slot_SS,MACinfo.REs_DMRS,MACinfo.Overhead_higherlayer,
									MACinfo.nPRB,MACinfo.coderateMCS,MACinfo.QAM_order,
									MACinfo.num_layers,MAXCB_KCBg2,MAXCB_KCBg1, &MACinfo);
			...
			TBS_long=Calc_TBS(Symb_X_slot, MACinfo.REs_DMRS, MACinfo.Overhead_higherlayer,
									MACinfo.nPRB, MACinfo.coderateMCS, MACinfo.QAM_order,
									MACinfo.num_layers, MAXCB_KCBg2, MAXCB_KCBg1, &MACinfo);
		...
		*output14=MACinfo;
		...
		//printf("TS=%d: MACinfo ASK SAMPLES TO DATASOURCE: TBS (bytes) = %d\n", Ts, *output0);
		...
		*output18=MACinfo;
		...
		int real_mod_order=MACinfo.QAM_order;	
		MACinfo.QAM_order=2;
		...
		*output21=MACinfo;
		...
		MACinfo.QAM_order=real_mod_order;	
