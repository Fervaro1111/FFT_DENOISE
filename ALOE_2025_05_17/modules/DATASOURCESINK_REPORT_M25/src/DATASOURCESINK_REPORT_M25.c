...
#include "DATASOURCESINK_REPORT_M25_interfaces.h"
#include "DATASOURCESINK_REPORT_M25_functions.h"
#include "DATASOURCESINK_REPORT_M25.h"

//ALOE Module Defined Parameters. Do not delete.
char mname[STR_LEN]="DATASOURCESINK_REPORT_M25";
int block_length=36;
...
int work(input_t *inp, output_t *out) {
	...
	datasourcesink_t *reportOUT=out(out,2);
...
//	if(rcv_samples2>0)	
//		printf("DATASOURCESINK_REPORT_M250: Tslot=%d, UserName=%s, ctrlVAL=%d, MAC_ID=%d\n", reportIN->Tslot, &reportIN->username[0], reportIN->ctrl_val, reportIN->MAC_ID);

	if(debug==1){
		printf("DATASOURCESINK_REPORT_M250: Tslot=%d || %s: rcv_samples0=%d, rcv_samples1=%d\n", Tslot, mname, rcv_samples0, rcv_samples1);
	}
	...
	if(ctrl_flow == 1){
		...
		if(rcv_samples1 > 0){
			...
			if(debug==1){
				printf("DATASOURCESINK_REPORT_M250: Tslot=%d || %s: snd_samples0=%d\n", Tslot, mname, snd_samples0);
			}
...
*/
	...
	// UPDATE REPORT
	update_CTRL_DATASOURCESINK(reportOUT, BER, RECV_THROUGHTPUT, RECV_SUBFRAMES, TimeSlot_us, &myfilename[0]);
	snd_samples2=ceil((float)sizeof(datasourcesink_t)/(float)sizeof(output_t));
