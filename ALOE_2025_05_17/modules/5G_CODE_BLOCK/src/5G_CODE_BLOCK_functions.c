...
//	printf("B=%d, L=%d, K1=%d, R=%f,A=%d\n",B,L,K1,R,CBinfoL->A);
	...
	if(NofExecs < 4){
		printf("############################# %16s  Please, get the values!!! ###################################\n", mname);
		printf("CODEBLOCKS  NofCodeBlocks=%d, CodeBlockSize=%d, CodeBlockDataSize=%d, CBCRCsize=%d, BGraph=%d, ILS=%d, Zc=%d\n", 
											C, CBinfoL->CBsize, CBinfoL->K1, CBinfoL->L_CB,CBinfoL->LDPCgraph, ILS, Zc);
		printf("############################# ############################################## ################################\n");
	}
...
int MAC_Buffer_Manager(cbctrl_t* CB_ctrl, MAC_struc* MAC_buff, int Ts){


	printf(">>>>>>>>>>>>MAC_Buffer_Manager(1): MAC_buff[Ts=%d]->Ts_MAC=%d\n", Ts, MAC_buff->Ts_MAC);
	if(Ts == MAC_buff->Ts_MAC){
		CB_ctrl->CodeRate=MAC_buff->coderateMCS;
		CB_ctrl->TB_CRC=MAC_buff->TB_size_TBCRC;
		printf("MAC_Buffer_Manager(2): Ts=%d, CB_ctrl->CodeRate=%f, CB_ctrl->TB_CRC=%d\n", Ts, CB_ctrl->CodeRate, CB_ctrl->TB_CRC);
		return(0);
	}
...
int AlignPipelineExec(int rcv_MACsamples, int rcv_bytes, cbctrl_t* CB_ctrl, MAC_struc *inputMAC){

	static MAC_struc MAC_buff[MAC_BUFFER];
	static int Ts=0;
	static int TsMAC=0;

//	printf("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT Ts=%d\n", Ts);

	// Manage MAC buffer
	if (rcv_MACsamples){
		//MAC_buff[TsMAC]=*inputMAC;
		memcpy(&MAC_buff[TsMAC], inputMAC, sizeof(MAC_struc));
//		printf("TTTTTTTTTTTTTTTTT %s rcv_bytes=%d, rcv_MACsamples=%d, MAC_buff[TsMAC=%d].Ts_MAC=%d\n", 
//					"CODE_BLOC.AlignPipelineExec", rcv_bytes, rcv_MACsamples,  TsMAC, MAC_buff[TsMAC].Ts_MAC);
		TsMAC++;
		if(TsMAC == MAC_BUFFER)TsMAC=0;
	}
//	printf("TTTTTTTTTrcv_bytes=%d\n", rcv_bytes);
	if(rcv_bytes == 0)return(0);

	// Check if MAC buffer allow operation
//	printf("TTTTTTTTTTTTTMAC_Buffer_Manager(1): MAC_buff[Ts=%d]->Ts_MAC=%d\n", Ts, MAC_buff->Ts_MAC);
	if(Ts == MAC_buff[Ts].Ts_MAC){
		CB_ctrl->CodeRate=MAC_buff[Ts].coderateMCS;
		CB_ctrl->TB_CRC=MAC_buff[Ts].TB_size_TBCRC;
//		printf("Ts == TsMAC  TTTTTTTTTTTTTTMAC_Buffer_Manager(2): Ts=%d, TsMAC=%d, CB_ctrl->CodeRate=%f, CB_ctrl->TB_CRC=%d\n", 
//					Ts, MAC_buff[Ts].Ts_MAC, CB_ctrl->CodeRate, CB_ctrl->TB_CRC);

		// Update Ts only if success
		Ts++;
		if(Ts==MAC_BUFFER)Ts=0;
...
int AlignPipelineExecRX(int rcv_MACsamples, int rcv_bytes, cbctrl_t* CB_ctrl, MAC_struc *inputMAC){
	...
	if (rcv_MACsamples){
		//MAC_buff[TsMAC]=*inputMAC;
		memcpy(&MAC_buff[TsMAC], inputMAC, sizeof(MAC_struc));
//		printf("GGGGGGGGGGGGGGGGGGG %s rcv_bytes=%d, rcv_MACsamples=%d, MAC_buff[TsMAC=%d].Ts_MAC=%d\n", 
//					"CODE_BLOC.AlignPipelineExec", rcv_bytes, rcv_MACsamples,  TsMAC, MAC_buff[TsMAC].Ts_MAC);
		TsMAC++;
		if(TsMAC == MAC_BUFFER)TsMAC=0;
	}
//	printf("GGGGGGGGGrcv_bytes=%d\n", rcv_bytes);
	if(rcv_bytes == 0)return(0);

	// Check if MAC buffer allow operation
//	printf("GGGGGGGGGGGGGGGGGGGMAC_Buffer_Manager(1): MAC_buff[Ts=%d]->Ts_MAC=%d\n", Ts, MAC_buff->Ts_MAC);
	if(Ts == MAC_buff[Ts].Ts_MAC){
		CB_ctrl->CodeRate=MAC_buff[Ts].coderateMCS;
		CB_ctrl->TB_CRC=MAC_buff[Ts].TB_size_TBCRC;
//		printf("Ts == TsMAC GGGGGGGGGGGGGGGGGGGTMAC_Buffer_Manager(2): Ts=%d, TsMAC=%d, CB_ctrl->CodeRate=%f, CB_ctrl->TB_CRC=%d\n", 
//						Ts, MAC_buff[Ts].Ts_MAC, CB_ctrl->CodeRate, CB_ctrl->TB_CRC);
