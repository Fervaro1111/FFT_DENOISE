...
int work(input_t *inp, output_t *out) {
	...
	if(strcmp(outputtype, "COMPLEXFLOAT")==0){
		...
		if(strcmp(signaltype, "TWO_TONE_COMPLEX")==0){
			gen_toneCOMPLEX(bufferA, tableA, TABLESZ, block_length, samplingfreqHz-14000.0, samplingfreqHz);
			gen_toneCOMPLEX2(bufferB, tableA, TABLESZ, block_length, samplingfreqHz-11000.0, samplingfreqHz);
			gen_toneCOMPLEX4(bufferC, tableA, TABLESZ, block_length, samplingfreqHz-500.0, samplingfreqHz);
			gen_toneCOMPLEX3(bufferD, tableA, TABLESZ, block_length, 500.0, samplingfreqHz);
			gen_toneCOMPLEX4(bufferE, tableA, TABLESZ, block_length, 14000.0, samplingfreqHz);
