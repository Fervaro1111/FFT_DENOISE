...
#include "MODULATOR_interfaces.h"
#include "MODULATOR_functions.h"
#include "MODULATOR.h"

//ALOE Module Defined Parameters. Do not delete.
char mname[STR_LEN]="MODULATOR";
int run_times=1;
...
int initialize() {
	...
	if (block_length > get_input_max_samples()) {
		...
		moderror("Check your MODULATOR_interfaces.h file\n");
		return -1;
	}
	/*Include the file name and line number when printing*/
...
int work(input_t *inp, output_t *out) {
	...
	int rcv_samples1 = get_input_samples(1); // number of samples at itf
...
int snd_samples=0;
	...
	printf("SURT del bufferRX_DEL_MODULADOR=%d \n",buff_out);
	//mod_order=buff_out;
	...
	Byte2bit(input,bitsOUTaux,rcv_samples);
...
///////////////////////////////Modulation selection//////////////////////////
	if (mod_order==1)snd_samples=mod_BPSK (bitsOUTaux, rcv_samples*8, output0);

	if (mod_order==2)snd_samples=mod_4QAM (bitsOUTaux, rcv_samples*8, output0);
	
	if (mod_order==4)snd_samples=mod_16QAM(bitsOUTaux, rcv_samples*8, output0);
...
//	printf("%1f + %1f I\n",creal(output0[i]),cimag(output0[i]));	
//	}
...
/*
for(i=0;i<snd_samples;i++){	
printf("symb in mod=%f + %f I \n",creal(output0[i]),cimag(output0[i]));
}
*/	
/*
	for(i=0;i<snd_samples;i++){
	printf("%1f + %1f I\n",creal(bytesOUT[i]),cimag(bytesOUT[i]));	
	}
*/
	
	//printf("snd_Samples_demod=%d\n",snd_samples_demod);
	//for(i=0;i<snd_samples;i++){
	//printf("%f + %f I\n",creal(bytesOUT[i]),cimag(bytesOUT[i]));	
	//}
