...
#include "BUFFER_PDCCH_interfaces.h"
#include "BUFFER_PDCCH_functions.h"
#include "BUFFER_PDCCH.h"

//ALOE Module Defined Parameters. Do not delete.
char mname[STR_LEN]="BUFFER_PDCCH";
int run_times=1;
...
float bufferB[2048];
int opMode=0; //Mode 0 means bypass, Mode 1 means buffer controlled by flag from 		      //MAC LAYER
...
int initialize() {
	...
	param_get_int("desalineacio", &desalineacio);
	...
	if (block_length > get_input_max_samples()) {
		...
		moderror("Check your BUFFER_PDCCH_interfaces.h file\n");
		return -1;
	}
	/*Include the file name and line number when printing*/
...
int work(input_t *inp, output_t *out) {
	...
	if (rcv_samples0 == 0 && rcv_samples1==0)return(0);
	// Check if data exceed maximum expected data
	printf("%s IN: rcv_samples0=%d\n", mname, rcv_samples0);
	printf("%s IN: rcv_samples1=%d\n", mname, rcv_samples1);

	/* do DSP stuff here */

	
	snd_samples=DO_Buffer(input0, input1, out,rcv_samples0, rcv_samples1, desalineacio);
