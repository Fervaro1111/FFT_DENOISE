...
#include "CRC_REPORT_interfaces.h"
#include "CRC_REPORT_functions.h"
#include "CRC_REPORT.h"

//ALOE Module Defined Parameters. Do not delete.
char mname[STR_LEN]="CRC_REPORT";
...
//		printf("%s: No data received\n", mname);
		...
		//ADD CRC
		if(mode==ADD__CRC){
			add_CRC(inp, rcv_samples0, &crc_p);
			snd_samples0=rcv_samples0+crcLengthBytes;
			memcpy(out, inp, sizeof(char)*snd_samples0);
	...
	*/
		...
		if(mode==CHECKCRC){
			numblocksrcv++;
			crc=check_CRC(inp, rcv_samples0, &crc_p);
...
*/
	...
	// UPDATE REPORT
	if(snd_samples0>0)CRC_Tslot=Tslot;
	if(rcv_samples1>0){	
		if(numblocksrcv>0)BLER=((float)numblocksNOK)/((float)numblocksrcv);
		else BLER=1.0;
		//printf("UNCRC rcv_samples1=%d, BLER=%1.6f\n", rcv_samples1, BLER);	
		update_CTRL_CRC(reportIN, reportOUT, BLER, numblocksrcv, CRC_Tslot);
		//snd_samples1=(sizeof(report_t)/sizeof(output_t));
