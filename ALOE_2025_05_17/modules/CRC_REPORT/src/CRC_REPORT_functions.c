...
int check_CRC(char *bufptr, int datalen, crc_t *crc_params){
	...
	int ALLZERO=0;
	//CHECK CRC
	for(i=0; i<datalen; i++)if(*(bufptr+i) == 0x00)ALLZERO=1;

	memcpy(crc_params->data0, bufptr, sizeof(char)*datalen);
	crc=crctable((unsigned long)datalen, crc_params);

//	if(ALLZERO==1 && crc==0x00)printf("ALLZERO==1 && crc==0x00\n");
...
//		printf("CRC_REPORT: Tslot=%d, UserName=%s, ctrlVAL=%d, MAC_ID=%d\n", reportIN->Tslot, &reportIN->username[0], reportIN->ctrl_val, reportIN->MAC_ID);
		memcpy(reportOUT, reportIN, sizeof(report_t));
//		printf("1-CRC_REPORT.update_CTRL(): BLER=%3.1f\n", reportOUT->BLER);
		reportOUT->BLER=BLER;
		reportOUT->subframe=subframe;
		reportOUT->CRC_Tslot=CRC_Tslot;
//		printf("2-CRC_REPORT.update_CTRL(): BLER=%3.1f\n", reportOUT->BLER);
}
