...
#include "5G_POLAR_ENC_interfaces.h"
#include "5G_POLAR_ENC_functions.h"
#include "5G_POLAR_ENC.h"

//ALOE Module Defined Parameters. Do not delete.
char mname[STR_LEN]="5G_POLAR_ENC";
int run_times=1;
...
int initialize() {
	...
	if (block_length > get_input_max_samples()) {
		...
		moderror("Check your 5G_POLAR_ENC_interfaces.h file\n");
		return -1;
	}
	/*Include the file name and line number when printing*/
...
int work(input_t *inp, output_t *out) {
	...
	//printf("POLAR ENCODER rcv_samp MAC=%d, in from MAC=%d\n",rcv_samples_MAC,*input1);
	...
	if(config_flag==0 && rcv_samples_MAC!=0 && PBCH_PDCCH==0){
		...
		int E=calc_out_bits_max_length(*input1,*input2); //El 2 son els symbols del coreset
	...
	//printf("%s IN: rcv_samples in bytes=%d\n", mname, rcv_samples);
	rcv_samples=rcv_samples*8;
	snd_samples=encode(input0,output0,rcv_samples);
