...
//		printf("1-DATASOURCESINK_REPORT.update_CTRL(): BER=%3.1f\n", reportOUT->BER);
		reportOUT->BER=BER;
		reportOUT->throughput=throughput;
//		reportOUT->DSS_Tslot=DSS_Tslot;
		reportOUT->IntDec=INTDEC;
		reportOUT->Coderate=CodeRate;
		reportOUT->MODidx=MODindex;
		reportOUT->MeasSubframeTsLength=dataPeriod;
		reportOUT->StandDevSubframeLength=Deviation;
		reportOUT->TimeSlot_us=TimeSlot_us;
		reportOUT->DSSRoundTripDelay=RoundTripDelay;
		reportOUT->DSSsndSamples=DSS_snd_samples0;
		strcpy(&reportOUT->file_name[0], filename);

//	printf("2-DATASOURCESINK_REPORT(): reportOUT->CHANNEL_Tslot=%d\n", reportOUT->CHANNEL_Tslot);
//		printf("2-DATASOURCESINK_REPORT.update_CTRL(): BER=%3.1f\n", reportOUT->BER);
}
