...
#include "CICLIC_PREFIX_interfaces.h"
#include "CICLIC_PREFIX_functions.h"
#include "CICLIC_PREFIX.h"

//ALOE Module Defined Parameters. Do not delete.
char mname[STR_LEN]="CICLIC_PREFIX";
int run_times=1;
...
int initialize() {
	...
	if (block_length > get_input_max_samples()) {
		...
		moderror("Check your CICLIC_PREFIX_interfaces.h file\n");
		return -1;
	}
	/*Include the file name and line number when printing*/
...
//	int i,k;
	...
	FFT_size=FFT_length_calc(rcv_samples,numFFT);
	//printf("%s IN: FFT_size=%d\n", mname, FFT_size);



	if(TX_RX==0){
		ADD_cyclic_Prefix(input,output,rcv_samples,numFFT);
		snd_samples=rcv_samples+FFT_size;
		set_output_samples(1, snd_samples*sizeof(_Complex float));
		//test//
		//snd_samples=snd_samples*4;
		//test//
	}else{
	
	REMOVE_cyclic_Prefix(input,output,rcv_samples,numFFT);
		snd_samples=rcv_samples-FFT_size;
	}
