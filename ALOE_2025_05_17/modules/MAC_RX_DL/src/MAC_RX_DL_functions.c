...
int MCS_selected(float coderate){

	for(int i=0;i<32;i++){//32 is number of entries in MCS table
		if((float)CodeRate51311[i]/1024.0==coderate) return i;
	}
...
void GETFFT_size(MAC_struc* MACinfo){
	if (MACinfo->nPRB<26)MACinfo->FFTsize=512;
	else if (MACinfo->nPRB>25 && MACinfo->nPRB<80)MACinfo->FFTsize=1024;
	else if (MACinfo->nPRB>79 && MACinfo->nPRB<161)MACinfo->FFTsize=2048;
	else if (MACinfo->nPRB>160 && MACinfo->nPRB<271) MACinfo->FFTsize=4096;
